...
def validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not str(config.get("timezone", "")).strip():
    ...
    decay = config.get("strike_decay_days", {})
    if not isinstance(decay, dict):
        decay = {}
    config["strike_decay_days"] = _deep_merge_config(DEFAULT_CONFIG["strike_decay_days"], decay)
...
async def reinstate_staff_command(
    ...
    try:
        reg_embed = discord.Embed(
            title="You were reinstated",
            description="Your staff record has been reactivated in the staff management database.",
            colour=discord.Colour.green(),
        )
        reg_embed.add_field(name="Minecraft Name", value=f"`{minecraft_name or existing['minecraft_name'] or 'None'}`", inline=True)
        reg_embed.add_field(name="Auto-Strike Eligible", value="Yes" if strike_eligible else "No", inline=True)
        reg_embed.set_footer(text="Your weekly activity, playtime, and staff records are being tracked again.")
        await member.send(embed=reg_embed)
        dm_sent = True
    except Exception:
        dm_sent = False
    ...
    await interaction.response.send_message(
        f"Reinstated {member.mention}." + (" DM sent." if dm_sent else " Could not send DM."),
        ephemeral=True,
    )
