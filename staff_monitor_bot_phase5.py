...
def add_strike(
    ...
    with get_db() as conn:
        conn.execute(
            "INSERT INTO strikes (discord_id, amount, reason, kind, issued_by, created_at, evidence_url) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (discord_id, amount, reason, kind, issued_by, now, evidence_url),
        )
...
async def apply_strike_and_notify(
    ...
    total = add_strike(member.id, amount, reason, kind, issued_by, evidence_url=evidence_url)
    if send_dm:
...
async def strike_add(
    ...
    await interaction.response.send_message(
        f"Issued {amount} strike(s) to {member.mention}. Reason: {reason}. Active strikes: {total}.{extra}",
        ephemeral=True,
    )
...
async def incident_log(
    ...
    view = StaffStatsButtonView(member.id, allow_remove_strike=False, evidence_url=evidence) if member else None
