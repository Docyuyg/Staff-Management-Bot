...
def _translate_sql_for_postgres(query: str) -> tuple[str, bool]:
    sql = query.strip()
    sql = sql.replace("?", "%s")

    if sql.startswith("INSERT OR IGNORE INTO processed_webhook_messages"):
        sql = sql.replace("INSERT OR IGNORE INTO", "INSERT INTO", 1) + " ON CONFLICT (message_id) DO NOTHING"
    elif sql.startswith("INSERT OR IGNORE INTO weekly_stats"):
        sql = sql.replace("INSERT OR IGNORE INTO", "INSERT INTO", 1) + " ON CONFLICT (week_key, discord_id) DO NOTHING"
    ...
    if upper_sql.startswith("INSERT INTO "):
        ...
        if table_name in serial_tables and "RETURNING" not in upper_sql:
            sql += " RETURNING id"
            returning_id = True
...
class PGConnectionWrapper:
    ...
    def execute(self, query: str, params: tuple[Any, ...] | list[Any] | None = None) -> PGCursorWrapper:
        sql, returning_id = _translate_sql_for_postgres(query)
...
CREATE TABLE IF NOT EXISTS staff_members (
    ...
    strike_eligible INTEGER NOT NULL DEFAULT 0,
    notes TEXT,
    last_minecraft_login_at TIMESTAMPTZ,
    inactivity_strike_anchor TIMESTAMPTZ,
    is_active INTEGER NOT NULL DEFAULT 1,
    offboard_type TEXT,
    offboard_reason TEXT,
    offboard_evidence_url TEXT,
    offboarded_by BIGINT,
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    discord_id BIGINT NOT NULL REFERENCES staff_members(discord_id) ON DELETE CASCADE,
    source TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS incident_logs (
    id BIGSERIAL PRIMARY KEY,
    subject_discord_id BIGINT REFERENCES staff_members(discord_id) ON DELETE SET NULL,
    severity TEXT NOT NULL,
    summary TEXT NOT NULL,
    details TEXT,
    evidence_url TEXT,
...
def init_db() -> None:
    with get_db() as conn:
        ...
        if not USE_POSTGRES:
            for sql in [
                "ALTER TABLE strikes ADD COLUMN evidence_url TEXT",
                "ALTER TABLE strikes ADD COLUMN is_permanent INTEGER NOT NULL DEFAULT 0",
                "ALTER TABLE staff_members ADD COLUMN is_active INTEGER NOT NULL DEFAULT 1",
                "ALTER TABLE staff_members ADD COLUMN offboard_type TEXT",
                "ALTER TABLE staff_members ADD COLUMN offboard_reason TEXT",
                "ALTER TABLE staff_members ADD COLUMN offboard_evidence_url TEXT",
                "ALTER TABLE staff_members ADD COLUMN offboarded_by INTEGER",
                "ALTER TABLE staff_members ADD COLUMN offboarded_at TEXT",
            ]:
                try:
                    conn.execute(sql)
                except Exception:
