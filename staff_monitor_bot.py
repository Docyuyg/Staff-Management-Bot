...
async def send_embed_to_channel(channel_id: int, embed: discord.Embed) -> None:
    if not channel_id:
        return
...
async def send_log(embed: discord.Embed) -> None:
    await send_embed_to_channel(resolve_channel_id("staff_log_channel_id"), embed)


async def send_loa_log(embed: discord.Embed) -> None:
    await send_embed_to_channel(resolve_channel_id("loa_request_channel_id", "staff_log_channel_id"), embed)
...
def build_loa_request_embed(row: sqlite3.Row) -> discord.Embed:
    ...
    embed.add_field(name="Review", value=f"/loa approve request_id:{row['id']}\n/loa deny request_id:{row['id']}", inline=False)
    return embed
...
async def loa_request(interaction: discord.Interaction, start_date: str, end_date: str, reason: str) -> None:
    ...
    if row:
        await send_loa_log(build_loa_request_embed(row))
...
async def loa_approve(interaction: discord.Interaction, request_id: int) -> None:
    ...
    await send_loa_log(build_loa_update_embed(f"LOA Approved • Request #{request_id}", row))
