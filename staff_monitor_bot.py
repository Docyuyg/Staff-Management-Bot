...
def inactive_staff_summary(staff: sqlite3.Row | None) -> str | None:
    if not staff or is_staff_active_row(staff):
        return None
    offboard_type = str(staff["offboard_type"] or "Offboarded")
    reason = str(staff["offboard_reason"] or "No reason recorded.")
    when = format_history_dt(staff["offboarded_at"]) if "offboarded_at" in staff.keys() else "Unknown"
    return f"**Status:** Inactive / {offboard_type}\n**Offboarded:** {when}\n**Reason:** {reason}"
...
def build_staff_profile_embed(member: discord.Member) -> discord.Embed | None:
    ...
    active_staff = is_staff_active_row(staff)
    current_status = status_label_for(member.id, int(current_stats["minecraft_seconds"]), bool(staff["strike_eligible"]), week_key_for()) if active_staff else "Inactive / Offboarded"
    current_reason = recommendation_reason_for(member.id, int(current_stats["minecraft_seconds"]), bool(staff["strike_eligible"]), week_key_for()) if active_staff else "This staff member is no longer active in the tracker."
    ...
    status_prefix = f"{status_emoji(current_status)} " if active_staff else ""
    standing_value = (
        f"**Status:** {status_prefix}{current_status}\n"
    ...
    inactive_summary = inactive_staff_summary(staff)
    if inactive_summary:
        embed.add_field(name="Inactive Record", value=inactive_summary[:1024], inline=False)
    embed.add_field(
...
def build_staff_activity_history_embed(member: discord.Member) -> discord.Embed | None:
    ...
    current_stats = fetch_weekly_stats(member.id)
    active_staff = is_staff_active_row(staff)
    status = status_label_for(member.id, int(current_stats["minecraft_seconds"]), bool(staff["strike_eligible"]), week_key_for()) if active_staff else "Inactive / Offboarded"
    ...
    status_prefix = f"{status_emoji(status)} " if active_staff else ""
    embed.add_field(
        name="Current Standing",
        value=(
            f"**Current Status:** {status_prefix}{status}\n"
...
def build_staff_stats_embed(member: discord.Member, staff: sqlite3.Row, stats: sqlite3.Row) -> discord.Embed:
    ...
    active_staff = is_staff_active_row(staff)
    status = status_label_for(member.id, int(stats["minecraft_seconds"]), bool(staff["strike_eligible"]), week_key_for()) if active_staff else "Inactive / Offboarded"
    ...
    status_prefix = f"{status_emoji(status)} " if active_staff else ""
    overview_value = (
        f"**Status:** {status_prefix}{status}\n"
        f"**Active Strikes:** {strikes}\n"
        f"**LOA:** {format_loa_row(loa_row)}"
    )
    if not active_staff:
        overview_value += f"\n**Offboard Type:** {staff['offboard_type'] or 'Offboarded'}"
    embed.add_field(
        name="Overview",
...
async def note_set(interaction: discord.Interaction, member: discord.Member, notes: str) -> None:
    ...
    embed = discord.Embed(title="Staff Note Updated", colour=discord.Colour.blurple())
...
async def note_clear(interaction: discord.Interaction, member: discord.Member) -> None:
    ...
    embed = discord.Embed(title="Staff Note Cleared", colour=discord.Colour.blurple())
    embed.add_field(name="Staff", value=member.mention, inline=False)
