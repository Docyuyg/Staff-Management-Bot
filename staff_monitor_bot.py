...
DEFAULT_CONFIG = {
    ...
    "staff_record_channel_id": 0,
    "strike_log_channel_id": 0,
    "incident_log_channel_id": 0,
    "case_log_channel_id": 0,
    ...
    "management_role_ids": [],
    "case_close_role_ids": [],  # sr admin roles allowed to close cases
...
def validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    config["case_close_role_ids"] = _normalize_unique_int_list(config.get("case_close_role_ids", []))
...
def standardize_log_embed(embed: discord.Embed, category: str) -> discord.Embed:
    footer_map = {
        "discipline": "Staff Management • Discipline Log",
        "investigation": "Staff Management • Investigation Log",
        "records": "Staff Management • Record Log",
        "loa": "Staff Management • LOA Log",
        "system": "Staff Management • System Log",
        "reports": "Staff Management • Report",
    }
    embed.set_footer(text=footer_map.get(category, "Staff Management"))
    return embed
...
def offboard_status_label(staff: sqlite3.Row) -> str:
    offboard_type = str(staff["offboard_type"] or "").strip()
    if offboard_type:
        return f"Inactive / Offboarded ({offboard_type.title()})"
    return "Inactive / Offboarded"


def offboard_status_note(staff: sqlite3.Row) -> str:
    parts: list[str] = []
    if staff["offboard_reason"]:
        parts.append(f"Reason: {staff['offboard_reason']}")
    if staff["offboarded_at"]:
        parts.append(f"When: {format_history_dt(staff['offboarded_at'])}")
    if not parts:
        return "This staff record is inactive and no longer part of active tracking."
    return " • ".join(parts)


def display_status_for_staff(staff: sqlite3.Row, discord_id: int, mc_seconds: int, wk: str) -> tuple[str, str]:
    if not is_staff_active_row(staff):
        return offboard_status_label(staff), offboard_status_note(staff)
    status = status_label_for(discord_id, mc_seconds, bool(staff["strike_eligible"]), wk)
    reason = recommendation_reason_for(discord_id, mc_seconds, bool(staff["strike_eligible"]), wk)
...
def build_case_embed(case_id: int) -> discord.Embed | None:
    ...
    embed.set_footer(text=f"Staff Management • Investigation Log • Created {format_history_dt(row['created_at'])}")
    return embed
...
class CaseLogView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if not interaction.guild or not isinstance(interaction.user, discord.Member) or not is_allowed_guild(interaction.guild):
            ...
            return False
        if not is_management_tier(interaction.user):
...
def build_staff_profile_embed(member: discord.Member) -> discord.Embed | None:
    ...
    current_status, current_reason = display_status_for_staff(
        staff,
        member.id,
        int(current_stats["minecraft_seconds"]),
        week_key_for(),
    )
    ...
    if not is_staff_active_row(staff):
        embed.add_field(
            name="Offboard Details",
            value=offboard_status_note(staff)[:1024],
            inline=False,
        )
    if staff["notes"]:
...
def build_staff_history_embed(member: discord.Member) -> discord.Embed | None:
    ...
    if not is_staff_active_row(staff):
        embed.add_field(name="Record Status", value=offboard_status_note(staff)[:1024], inline=False)
...
def build_staff_activity_history_embed(member: discord.Member) -> discord.Embed | None:
    ...
    minimum_seconds = int(float(CONFIG["minimum_weekly_hours"]) * 3600)
    current_stats = fetch_weekly_stats(member.id)
    status, status_reason = display_status_for_staff(
        staff,
        member.id,
        int(current_stats["minecraft_seconds"]),
        week_key_for(),
...
def is_founder_tier(member: discord.Member) -> bool:
    return is_management(member)


def is_management_tier(member: discord.Member) -> bool:
    return is_management(member) or is_upper_staff(member)


def can_close_cases(member: discord.Member) -> bool:
    if is_founder_tier(member):
        return True
    close_role_ids = set(CONFIG.get("case_close_role_ids", []))
    return any(role.id in close_role_ids for role in member.roles)


def is_registered_staff_member(member: discord.Member) -> bool:
    row = get_staff_member(member.id)
    return bool(row) and is_staff_active_row(row)
...
async def send_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("staff_record_channel_id", "staff_log_channel_id"), embed, mention_management=True, view=view)


async def send_staff_record_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("staff_record_channel_id", "staff_log_channel_id"), embed, mention_management=True, view=view)


async def send_strike_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("strike_log_channel_id", "staff_record_channel_id"), embed, mention_management=True, view=view)


async def send_incident_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("incident_log_channel_id", "strike_log_channel_id"), embed, mention_management=True, view=view)


async def send_case_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("case_log_channel_id", "incident_log_channel_id"), embed, mention_management=True, view=view)
...
async def send_offboard_log(embed: discord.Embed, *, view: discord.ui.View | None = None) -> None:
    await send_embed_to_channel(resolve_channel_id("offboard_log_channel_id", "staff_record_channel_id"), embed, mention_management=True, view=view)
...
def build_role_sync_embed(guild: discord.Guild) -> discord.Embed:
    ...
    if not issues:
        ...
        embed.description = "No role/database mismatches are currently active."
        return standardize_log_embed(embed, "system")

    embed = discord.Embed(title="Role Sync Audit", colour=discord.Colour.orange())
    embed.description = f"Found **{len(issues)}** role/database mismatch issue(s) that should be reviewed."
    chunks: list[str] = []
...
def build_management_reminders_embed(guild: discord.Guild, wk: str) -> discord.Embed | None:
    ...
    embed = discord.Embed(title="Management Reminders", colour=discord.Colour.orange())
    if loa_endings:
        lines = []
    ...
    embed.set_footer(text=f"Staff Management • System Log • Week key: {wk}")
    return embed
...
async def apply_strike_and_notify(
    ...
    embed = discord.Embed(title="Staff Strike Logged", colour=discord.Colour.red())
    embed.description = "A strike was added to a staff record."
    embed.add_field(name="Staff", value=f"{member} ({member.id})", inline=False)
    embed.add_field(name="Amount", value=str(amount), inline=True)
    embed.add_field(name="Active Strikes", value=str(total), inline=True)
    embed.add_field(name="Type", value=kind.replace("_", " ").title(), inline=True)
    if issued_by:
        embed.add_field(name="Issued By", value=f"<@{issued_by}>", inline=False)
    embed.add_field(name="Reason", value=reason[:1024], inline=False)
    if evidence_url:
        embed.add_field(name="Evidence", value=evidence_url, inline=False)
    embed.add_field(name="Auto-Expires", value="No" if is_permanent else "Yes", inline=True)
    standardize_log_embed(embed, "discipline")
    await send_strike_log(embed, view=StaffStatsButtonView(member.id, allow_remove_strike=True, evidence_url=evidence_url))
    return total


async def founder_predicate(interaction: discord.Interaction) -> bool:
    ...
    if is_founder_tier(interaction.user):
        return True

    raise app_commands.CheckFailure("Only founder-level staff can use this command.")
...
async def management_predicate(interaction: discord.Interaction) -> bool:
    ...
    if is_management_tier(interaction.user):
        return True

    raise app_commands.CheckFailure("Only upper staff or founder-level staff can use this command.")


founder_check = app_commands.check(founder_predicate)
...
async def on_audit_log_entry_create(entry: discord.AuditLogEntry) -> None:
    ...
    try:
        ...
        await send_staff_record_log(embed, view=StaffStatsButtonView(entry.user.id, allow_remove_strike=False))
    except Exception:
...
@staff_group.command(name="register", description="Founder-only: register a new staff member in the tracker")
@founder_check
...
def build_loa_request_embed(row: sqlite3.Row) -> discord.Embed:
    ...
    return standardize_log_embed(embed, "loa")
...
class OverviewDashboardView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if not interaction.guild or not isinstance(interaction.user, discord.Member) or not is_allowed_guild(interaction.guild):
            ...
            return False
        if not is_management_tier(interaction.user):
            await interaction.response.send_message("Only upper staff or founder-level staff can use this button.", ephemeral=True)
            return False
        return True
...
def build_staff_stats_embed(member: discord.Member | discord.User) -> discord.Embed | None:
    ...
    status, status_reason = display_status_for_staff(
        staff,
        member.id,
        int(stats["minecraft_seconds"]),
        week_key_for(),
    )
    loa_row = get_active_loa_for_member(member.id)
    embed = discord.Embed(title=f"Staff Stats • {member}", colour=discord.Colour.blurple())
    ...
    embed.add_field(
        name="Record Status",
        value=("Active" if is_staff_active_row(staff) else offboard_status_label(staff)),
        inline=True,
    )
    if not is_staff_active_row(staff):
        embed.add_field(name="Offboard Details", value=offboard_status_note(staff)[:1024], inline=False)
    if staff["notes"]:
...
class StaffStatsButtonView(discord.ui.View):
    ...
    async def remove_one(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        embed = standardize_log_embed(discord.Embed(title="Staff Strike Removed", colour=discord.Colour.green()), "discipline")
        ...
        await send_strike_log(embed, view=StaffStatsButtonView(self.discord_id, allow_remove_strike=False))
...
class IncidentLogView(discord.ui.View):
    ...
    async def remove_incident(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if not row:
            ...
            return
        embed = standardize_log_embed(discord.Embed(title="Staff Incident Removed", colour=discord.Colour.green()), "investigation")
...
async def incident_log(
    ...
    evidence = evidence_url.strip() or None
    if severity.value == "severe" and not evidence:
        await interaction.response.send_message(
            "Severe incidents require an evidence link.",
            ephemeral=True,
        )
        return
    ...
    embed = discord.Embed(title=f"Staff Incident • #{incident_id}", colour=colour_map.get(severity.value, discord.Colour.orange()))
    if member:
...
async def note_set(interaction: discord.Interaction, member: discord.Member, notes: str) -> None:
    if not get_staff_member(member.id):
        ...
        return
    cleaned = notes.strip()
    set_staff_notes(member.id, cleaned, interaction.user.id)
    embed = standardize_log_embed(discord.Embed(title="Staff Note Updated", colour=discord.Colour.blurple()), "records")
    embed.add_field(name="Staff", value=member.mention, inline=False)
    embed.add_field(name="Updated By", value=f"<@{interaction.user.id}>", inline=True)
    embed.add_field(name="Note", value=cleaned[:1024], inline=False)
    await send_staff_record_log(embed)
...
async def note_clear(interaction: discord.Interaction, member: discord.Member) -> None:
    ...
    embed = standardize_log_embed(discord.Embed(title="Staff Note Cleared", colour=discord.Colour.blurple()), "records")
    embed.add_field(name="Staff", value=member.mention, inline=False)
    embed.add_field(name="Cleared By", value=f"<@{interaction.user.id}>", inline=True)
    await send_staff_record_log(embed)
...
async def offboard_staff_command(
    ...
    if offboard_type.value != "resigned" and not (evidence_url or "").strip():
        await interaction.response.send_message(
            "Offboards other than resignation require an evidence link.",
            ephemeral=True,
        )
        return
    ...
    embed.add_field(name="Offboarded By", value=f"<@{interaction.user.id}>", inline=True)
    embed.add_field(name="Reason", value=reason[:1024], inline=False)
    if evidence_url:
        embed.add_field(name="Evidence", value=evidence_url, inline=False)
    standardize_log_embed(embed, "records")
...
@staff_group.command(name="reinstate", description="Founder-only: reinstate an offboarded staff member")
@founder_check
...
async def reinstate_staff_command(
    ...
    embed.add_field(name="Reinstated By", value=f"<@{interaction.user.id}>", inline=False)
    standardize_log_embed(embed, "records")
...
@staff_group.command(name="role_sync_audit", description="Founder-only: audit role/database mismatches")
@founder_check
...
async def strike_remove(interaction: discord.Interaction, member: discord.Member, amount: app_commands.Range[int, 1, 10]) -> None:
    ...
    embed.add_field(name="Active Strikes", value=str(total))
    await send_strike_log(embed, view=StaffStatsButtonView(member.id, allow_remove_strike=False))
    await interaction.response.send_message(
...
async def loa_request(interaction: discord.Interaction, start_date: str, end_date: str, reason: str) -> None:
    if not interaction.guild or not is_allowed_guild(interaction.guild):
        ...
        return
    if not isinstance(interaction.user, discord.Member) or not is_registered_staff_member(interaction.user):
        await interaction.response.send_message("You are not registered as active staff in the tracker.", ephemeral=True)
        return
    try:
...
async def case_open_command(
    ...
    if severity.value == "severe" and not evidence:
        await interaction.response.send_message(
            "Severe cases require an evidence link.",
            ephemeral=True,
        )
        return
    ...
    if embed:
        await send_case_log(embed, view=view)
...
async def case_update_command(
    ...
    if status_after == "closed":
        await interaction.response.send_message(
            "Use `/staff case_close` to close a case.",
            ephemeral=True,
        )
        return
    evidence = evidence_url.strip() or None
    ...
    row = get_case(case_id)
    update_embed = discord.Embed(title=f"Staff Case Update • #{case_id}", colour=discord.Colour.orange())
    ...
    standardize_log_embed(update_embed, "investigation")
    await send_case_log(update_embed, view=CaseLogView(case_id, discord_id=int(row['subject_discord_id']) if row and row['subject_discord_id'] else None, evidence_url=evidence or (str(row['evidence_url']) if row and row['evidence_url'] else None)))
...
async def case_close_command(interaction: discord.Interaction, case_id: int, outcome: str, evidence_url: str = "") -> None:
    if not isinstance(interaction.user, discord.Member) or not can_close_cases(interaction.user):
        await interaction.response.send_message(
            "Only Sr. Admin and founder-level staff can close cases.",
            ephemeral=True,
        )
        return
    existing = get_case(case_id)
    if not existing:
        ...
        return
    evidence = evidence_url.strip() or None
    existing_evidence = str(existing["evidence_url"]) if existing["evidence_url"] else None
    if str(existing["severity"]).lower() == "severe" and not (evidence or existing_evidence):
        await interaction.response.send_message(
            "Severe cases require an evidence link before they can be closed.",
            ephemeral=True,
        )
        return
    ...
    standardize_log_embed(embed, "investigation")
    await send_case_log(embed, view=CaseLogView(case_id, discord_id=int(row['subject_discord_id']) if row['subject_discord_id'] else None, evidence_url=evidence or (str(row['evidence_url']) if row['evidence_url'] else None)))
