...
def build_management_ping_text(channel: discord.abc.GuildChannel | None) -> str | None:
    ...
    parts: list[str] = []
    for raw_id in CONFIG.get("management_ping_target_ids", []):
        try:
...
async def send_embed_to_channel(channel_id: int, embed: discord.Embed, *, mention_management: bool = False, view: discord.ui.View | None = None) -> None:
    if not channel_id:
        return
    ...
    if isinstance(channel, discord.abc.Messageable):
        ...
        if mention_management:
            content = build_management_ping_text(channel if isinstance(channel, discord.abc.GuildChannel) else None)
...
class ActiveLOAView(discord.ui.View):
    ...
    @discord.ui.button(label="Remove LOA Early", style=discord.ButtonStyle.secondary)
