...
@staff_group.command(name="backup_db", description="Download a backup of the live database")
@management_check
async def backup_db_command(interaction: discord.Interaction) -> None:
    await interaction.response.defer(ephemeral=True)
    try:
        if not DB_PATH.exists():
            await interaction.followup.send("Database file was not found on the live bot.", ephemeral=True)
            return
        timestamp = utcnow().astimezone(TZ).strftime("%Y%m%d_%H%M%S")
        data = DB_PATH.read_bytes()
        file = discord.File(io.BytesIO(data), filename=f"staff_monitor_backup_{timestamp}.db")
        await interaction.followup.send(
            content="Live database backup attached. Download this file and upload it back here.",
            file=file,
            ephemeral=True,
        )
    except Exception as exc:
        log.exception("backup_db failed")
        await interaction.followup.send(f"Backup failed: {type(exc).__name__}", ephemeral=True)
