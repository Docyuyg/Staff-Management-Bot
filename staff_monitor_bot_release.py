...
class CaseLogView(discord.ui.View):
    def __init__(self, case_id: int, *, discord_id: int | None = None, evidence_url: str | None = None):
...
def expire_decayed_strikes() -> int:
    ...
    with get_db() as conn:
        rows = conn.execute(
            "SELECT id, kind, reason, created_at, is_permanent FROM strikes WHERE active = 1"
        ).fetchall()
        for row in rows:
...
def compute_role_sync_issues(guild: discord.Guild) -> list[dict[str, str]]:
    ...
    for row in staff_rows:
        ...
        if is_active and bool(row["strike_eligible"]) and not has_lower:
            issues.append({
                "type": "Lower-staff mismatch",
                "who": member.mention,
                "details": "Marked strike-eligible in the bot but does not have a lower staff role.",
            })
...
def build_export_text_for_member(discord_id: int) -> str:
    ...
    def add_section(title: str, producer) -> None:
        lines.append("")
        lines.append(title)
        try:
            produced = producer()
            if isinstance(produced, list):
                ...
                lines.append(produced if produced else "None.")
            else:
                lines.append(str(produced))
        except Exception as exc:
            log.exception("Export section failed for discord_id=%s section=%s", discord_id, title)
            lines.append(f"[section failed: {type(exc).__name__}]")
    ...
    def current_snapshot_section():
        ...
        return [
            f"current_week_key: {week_key_for()}",
            f"current_week_playtime: {human_hours(int(current_stats['minecraft_seconds']))}",
            f"current_week_punishments: {total_p}",
            f"active_strikes: {active_strike_count(discord_id)}",
            f"permanent_active_strikes: {count_permanent_active_strikes(discord_id)}",
            f"recommendation: {status_label_for(discord_id, int(current_stats['minecraft_seconds']), bool(staff['strike_eligible']), week_key_for())}",
        ]

    def recent_week_snapshots_section():
        with get_db() as conn:
    ...
    add_section("=== Current Snapshot ===", current_snapshot_section)
    add_section("=== Recent Weekly Snapshots ===", recent_week_snapshots_section)
    add_section("=== Recent Strikes ===", recent_strikes_section)
    add_section("=== Recent Incidents ===", recent_incidents_section)
    add_section("=== Recent LOA ===", recent_loa_section)
    add_section("=== Recent Notes ===", recent_notes_section)
    add_section("=== Recent Weekly Activity ===", recent_activity_section)
...
staff_group = app_commands.Group(name="staff", description="Staff management")
mod_group = app_commands.Group(name="mod", description="Tracked moderation actions")
admin_group = app_commands.Group(name="admin", description="Internal tracking helpers")
loa_group = app_commands.Group(name="loa", description="Leave of absence")
...
async def register_staff(interaction: discord.Interaction, member: discord.Member, minecraft_name: str, strike_eligible: bool) -> None:
    ...
    msg = (
        f"{member.mention} was added to the staff database with Minecraft name `{minecraft_name}`. "
        f"Auto-strike eligible: `{'Yes' if strike_eligible else 'No'}`."
    )
    if dm_sent:
        msg += " Confirmation DM sent."
    else:
        msg += " Confirmation DM could not be sent."
...
def build_staff_stats_embed(member: discord.Member, staff: sqlite3.Row, stats: sqlite3.Row) -> discord.Embed:
    ...
    embed = discord.Embed(title=f"Staff Snapshot • {member}", colour=discord.Colour.blurple())
    embed.add_field(
        name="Overview",
        value=(
...
def build_private_leaderboard_embed(guild: discord.Guild, wk: str) -> discord.Embed | None:
    ...
    top_activity = sorted(scored, key=lambda x: (-x["mc_seconds"], x["strikes"], -x["streak"]))[:5]
    top_clean = sorted(
        [x for x in scored if x["strikes"] == 0],
        key=lambda x: (-x["streak"], -x["mc_seconds"], -x["punishments"])
    )[:5]
    if not top_clean:
        top_clean = sorted(scored, key=lambda x: (x["strikes"], -x["streak"], -x["mc_seconds"]))[:5]
    top_consistency = sorted(scored, key=lambda x: (-x["streak"], -x["mc_seconds"], x["strikes"]))[:5]
    ...
    embed = discord.Embed(
        title="Private Staff Leaderboard",
        description="Management/staff only. Current week snapshot.",
...
async def reinstate_staff_command(
    ...
    try:
        dm_embed = discord.Embed(title="Staff Status Updated", colour=discord.Colour.green())
        dm_embed.description = "You were reinstated in the staff database."
        dm_embed.add_field(name="Minecraft Name", value=f"`{minecraft_name or existing['minecraft_name'] or 'None'}`", inline=True)
        dm_embed.add_field(name="Auto-Strike Eligible", value="Yes" if strike_eligible else "No", inline=True)
...
@staff_group.command(name="export", description="Export a staff member's record")
@management_check
@app_commands.describe(member="Staff member to export")
async def export_staff_command(interaction: discord.Interaction, member: discord.Member) -> None:
    if not get_staff_member(member.id):
        await interaction.response.send_message("That staff member is not registered.", ephemeral=True)
        return
    try:
        data = build_export_text_for_member(member.id).encode("utf-8")
        file = discord.File(io.BytesIO(data), filename=f"staff_export_{member.id}.txt")
        await interaction.response.send_message(content=f"Export for {member.mention}", file=file, ephemeral=True)
    except Exception as exc:
        log.exception("staff export failed for discord_id=%s", member.id)
        await interaction.response.send_message(
            f"Export failed: {type(exc).__name__}. Check logs and try again.",
            ephemeral=True,
        )
...
async def case_view_command(interaction: discord.Interaction, case_id: int) -> None:
    ...
    if not embed:
        ...
        return
    row = get_case(case_id)
    view = CaseLogView(case_id, discord_id=int(row['subject_discord_id']) if row and row['subject_discord_id'] else None, evidence_url=(str(row['evidence_url']) if row and row['evidence_url'] else None))
