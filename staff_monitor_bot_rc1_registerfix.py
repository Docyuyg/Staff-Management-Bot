...
def validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(decay_cfg, dict):
        decay_cfg = {}
    merged_decay = {
        "minor": int(decay_cfg.get("minor", DEFAULT_CONFIG["strike_decay_days"]["minor"])),
        "major": int(decay_cfg.get("major", DEFAULT_CONFIG["strike_decay_days"]["major"])),
        "severe": int(decay_cfg.get("severe", DEFAULT_CONFIG["strike_decay_days"]["severe"])),
    }
    for severity, days in merged_decay.items():
        if days < 0:
            raise ValueError(f"strike_decay_days.{severity} cannot be negative.")
